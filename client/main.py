...
class Client():
    ...
    def upsert_device(self):
        ...
        if insert_device.status_code == 201:
            ...
            print('Device with this UUID found.')
            return True
    ...
    def process(self):
        if not self.upsert_device():
            ...
            print('Something went wrong while attempting to create the device...')

        if not self.upsert_software():
            # logging.error('Something went wrong with inserting the software...')
            print('Something went wrong while attempting to insert the software...')
