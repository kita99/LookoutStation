"""Initial migration.

Revision ID: c365b43165a1
Revises: 
Create Date: 2020-07-03 23:30:20.173994

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = 'c365b43165a1'
down_revision = None
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table('cve_software',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('cve_id', sa.Integer(), nullable=True),
    sa.Column('software_id', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['cve_id'], ['cves.id'], ondelete='cascade'),
    sa.ForeignKeyConstraint(['software_id'], ['software.id'], ondelete='cascade'),
    sa.PrimaryKeyConstraint('id')
    )
    op.add_column('ports', sa.Column('service_version', sa.String(length=50), nullable=True))
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_column('ports', 'service_version')
    op.drop_table('cve_software')
    # ### end Alembic commands ###
